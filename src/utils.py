...
def create_database(database_name: str):
    ...
    def create_tables(database_name):
        ...
        conn = psycopg2.connect(database_name=database_name, user=os.getenv("user"), password=os.getenv("password"),
                                host=os.getenv("host"), port=os.getenv("port"))
        with conn:
    ...
    with conn:
        with conn.cursor() as cur:
            ...
            for employer in employers:
                ...
                cur.execute("INSERT INTO employer VALUES (%s, %s, %s)", (employer_id, all_vacancies, employer["name"]))
